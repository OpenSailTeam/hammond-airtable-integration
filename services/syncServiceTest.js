...
module.exports = {
  ...
  syncToGoogleAds: async (records) => {
    for (const record of records) {
      ...
      try {
        console.log(`Calling getListingById(${record.fields["Webflow Item ID"]})`)
        if (await adsService.getListingById(record.fields["Webflow Item ID"])) {
          ...
          if (
            ...
            console.log(`Calling removeListingById(${record.fields["Webflow Item ID"]})`)
            await adsService.removeListingById(record.fields["Webflow Item ID"]);
          } else {
            // If record is found in feed and is not a draft or archived, update it
            console.log(`Calling updateListingDataById(${record.fields["Webflow Item ID"]}, ${fieldData})`)
            await adsService.updateListingDataById(record.fields["Webflow Item ID"], fieldData);
          }
        } else {
